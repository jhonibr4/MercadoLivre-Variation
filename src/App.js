...
textoSelecionado.remove();
  ...
  async function achar(){
    ...
    const response = await api.get(id.replace('-', ''))
    try{
  ...
  return (
    ...
    <div className="itemBox">
      <div className="containerImage">
        { img === null ? null : <>
          <img className="imgBox" src={img} alt="Imagem do produto" />
          
        </> }
      </div>
      { infos.length === 0 ?
      <>
      <Lottie className='lottie' animationData={NotFound} options={options}/>  
      <h1 className='textLottie'>Nenhum produto está na lista</h1>
      </> :
      <div>
        ...
        <div className="containerItem" key={info.id}>
         ...
         <div className="Itens">
           <div className="colunmItem">
             <span id="subItensID"> 
             <img src={Key} alt="Icon color" /><b>ID:</b> {info.id} 
             <button id="buttonCopy" onClick={() => copiarTexto(info.id)}><FiCopy id="iconCopy"/></button>
              <p className="subItens"><img src={Money} alt="Icon color" /> <b>Preço:</b> R${preco}</p>
             </span>
